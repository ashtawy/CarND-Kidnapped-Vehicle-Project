...
/* Multi_variate Gaussian Density for position
*/
inline double mv_gaussian(double mu_x, double std_x, double x, double mu_y, double std_y, double y){
	double exp_terms = -(((x-mu_x)*(x-mu_x))/(2*std_x*std_x) + ((y-mu_y)*(y-mu_y))/(2*std_y*std_y));
	return (1/(2*M_PI*std_x*std_y))*exp(exp_terms);
}
